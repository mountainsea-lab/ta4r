...
impl<T, CM, HM, S, I, R> Clone for AverageTrueRangeTrailingStopLossRule<T, CM, HM, S, I, R>
where
    ...
    T: 'static + Clone + TrNum,
{
    fn clone(&self) -> Self {
        Self {
            stop_loss_threshold: self.stop_loss_threshold.clone(),
            reference_price: self.reference_price.clone(),
