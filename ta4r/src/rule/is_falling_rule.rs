...
/// IsFallingRule: 指标在过去 N 根 Bar 内下跌比例达到 min_strength
pub struct IsFallingRule<T, CM, HM, S, IR, R>
where
...
impl<T, CM, HM, S, IR, R> IsFallingRule<T, CM, HM, S, IR, R>
where
    T: TrNum + Clone + 'static,
    CM: CostModel<T> + Clone,
    HM: CostModel<T> + Clone,
...
impl<T, CM, HM, S, IR, R> Rule for IsFallingRule<T, CM, HM, S, IR, R>
where
    T: TrNum + Clone + 'static,
    CM: CostModel<T> + Clone,
    HM: CostModel<T> + Clone,
