...
#[derive(Clone)]
pub struct Position<'a, N, CM, HM, S>
where
    N: TrNum,
...
{
    entry: Option<Trade<'a, N, CM, S>>,
    exit: Option<Trade<'a, N, CM, S>>,
...
impl<'a, N, CM, HM, S> Position<'a, N, CM, HM, S>
where
    N: TrNum,
    CM: CostModel<N> + Clone,
    HM: CostModel<N> + Clone,
    S: BarSeries<'a, N> + ?Sized,
{
    ...
    pub fn entry(&self) -> Option<&Trade<'a, N, CM, S>> {
        self.entry.as_ref()
    }

    pub fn exit(&self) -> Option<&Trade<'a, N, CM, S>> {
        self.exit.as_ref()
    }
    ...
    pub fn operate(&mut self, index: usize, price: N, amount: N) -> Option<Trade<'a, N, CM, S>> {
        if self.is_new() {
            let trade = Trade::new(
                index,
                self.starting_type,
    ...
    pub fn get_profit_with_final(&self, final_index: usize, final_price: N) -> N {
    ...
    pub fn get_gross_profit(&self, final_price: N) -> N {
    ...
    pub fn get_gross_return(&self, entry_price: N, exit_price: N) -> N {
