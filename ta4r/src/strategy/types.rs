...
/// ====================
#[derive(Clone)]
pub enum Strategies<S: Strategy> {
    Base(Arc<S>),
    And(Arc<Strategies<S>>, Arc<Strategies<S>>),
    Or(Arc<Strategies<S>>, Arc<Strategies<S>>),
    Opposite(Arc<Strategies<S>>),
}

impl<S: Strategy> Strategies<S> {
    pub fn and(self, other: Strategies<S>) -> Strategies<S> {
        Strategies::And(Arc::new(self), Arc::new(other))
    }
    pub fn or(self, other: Strategies<S>) -> Strategies<S> {
        Strategies::Or(Arc::new(self), Arc::new(other))
    }
    pub fn opposite(self) -> Strategies<S> {
        Strategies::Opposite(Arc::new(self))
    }

    pub fn name(&self) -> String {
        match self {
            Strategies::Base(s) => s.name().to_string(),
            Strategies::And(l, r) => format!("({} AND {})", l.name(), r.name()),
            Strategies::Or(l, r) => format!("({} OR {})", l.name(), r.name()),
            Strategies::Opposite(inner) => format!("NOT ({})", inner.name()),
        }
    }
    ...
    pub fn should_operate(&self, index: usize, trading_record: &S::TradingRec) -> bool {
        match self {
            Strategies::Base(s) => s.should_operate(index, trading_record),
            Strategies::And(l, r) => {
                l.should_operate(index, trading_record) && r.should_operate(index, trading_record)
            }
            Strategies::Or(l, r) => {
            ...
            Strategies::Opposite(inner) => !inner.should_operate(index, trading_record),
        }
    }
}
...
pub enum DynStrategies<R> {
    Base(Arc<dyn StrategyDyn<TradingRec = R>>),
    And(Arc<DynStrategies<R>>, Arc<DynStrategies<R>>),
    Or(Arc<DynStrategies<R>>, Arc<DynStrategies<R>>),
    Opposite(Arc<DynStrategies<R>>),
}
...
impl<R> DynStrategies<R> {
    ...
    where
        S: Strategy<TradingRec = R> + 'static,
    {
        DynStrategies::Base(Arc::new(DynStrategyAdapter::new(s)))
    }

    pub fn and(self, other: Self) -> Self {
        DynStrategies::And(Arc::new(self), Arc::new(other))
    }
    pub fn or(self, other: Self) -> Self {
        DynStrategies::Or(Arc::new(self), Arc::new(other))
    }
    pub fn opposite(self) -> Self {
        DynStrategies::Opposite(Arc::new(self))
    }
